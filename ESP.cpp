...
void CEsp::DrawBox(CEsp::ESPBox size, Color color)
{
	...
	//else
	{
		...
		Render::Clear(size.x, size.y - 1, VertLine, 1, Color(255, 255, 255, 150));
		Render::Clear(size.x + size.w - VertLine, size.y - 1, VertLine, 1, Color(255, 255, 255, 150));
		Render::Clear(size.x, size.y + size.h - 1, VertLine, 1, Color(255, 255, 255, 150));
		Render::Clear(size.x + size.w - VertLine, size.y + size.h - 1, VertLine, 1, Color(255, 255, 255, 150));

		Render::Clear(size.x - 1, size.y, 1, HorzLine, Color(255, 255, 255, 150));
		Render::Clear(size.x - 1, size.y + size.h - HorzLine, 1, HorzLine, Color(255, 255, 255, 150));
		Render::Clear(size.x + size.w - 1, size.y, 1, HorzLine, Color(255, 255, 255, 150));
		Render::Clear(size.x + size.w - 1, size.y + size.h - HorzLine, 1, HorzLine, Color(255, 255, 255, 150));
...
void CEsp::DrawHealth(IClientEntity* pEntity, CEsp::ESPBox size)
{
	...
	Verts[2].Init(Vector2D(HealthBar.x + size.w, HealthBar.y + 1));
	Verts[3].Init(Vector2D(HealthBar.x - 5, HealthBar.y + 1));

	Render::PolygonOutline(4, Verts, Color(50, 200, 50, 255), Color(100, 100, 100, 170));
	...
	Verts2[3].Init(Vector2D(HealthBar.x - 4, HealthBar.y + 2));
	...
	Render::Polygon(4, Verts2, Color(0, 0, 0, 40));
...
void CEsp::DrawCross(IClientEntity* pEntity)
{
	...
	{
		Render::Clear(screen.x - Scale, screen.y - (Scale * 2), (Scale * 2), (Scale * 4), Color(255, 255, 255, 160));
...
void CEsp::DrawDrop(IClientEntity* pEntity, ClientClass* cClass)
{
	...
	if (!plr && Render::WorldToScreen(Weapon->GetOrigin(), Box))
	{
		if (Menu::Window.VisualsTab.OptionsBox.GetState())
		{
			...
			Render::Outline(Box.x - 3, Box.y - 3, 6, 6, Color(255, 255, 255, 150));
		}
...
void CEsp::DrawChicken(IClientEntity* pEntity, ClientClass* cClass)
{
	...
	if (GetBox(pEntity, Box))
	{
		player_info_t pinfo; strcpy_s(pinfo.name, "stupid head");
		if (Menu::Window.VisualsTab.OptionsBox.GetState())
...
void CEsp::DrawBombPlanted(IClientEntity* pEntity, ClientClass* cClass) 
{
	...
	if (Render::WorldToScreen(vOrig, vScreen))
	{
		...
		Render::Text(vScreen.x, vScreen.y, Color(0, 255, 0, 255), Render::Fonts::ESP, buffer);
	}
}
...
void DrawBoneTest(IClientEntity *pEntity)
{
	for (int i = 0; i < 127; i++)
	{
		...
		if (Render::WorldToScreen(BoneLoc, BoneScreen))
		{
			char buf[10];
			_itoa_s(i, buf, 10);
			Render::Text(BoneScreen.x, BoneScreen.y, Color(255, 0, 0, 180), Render::Fonts::ESP, buf);
		}
	}
}
