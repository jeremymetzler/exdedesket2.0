...
void CGUI::Draw()
{
	...
	if (ShouldDrawCursor)
	{
		...
		MouseVt[1].Init(Vector2D(Mouse.x + 5, Mouse.y));
		MouseVt[2].Init(Vector2D(Mouse.x, Mouse.y + 5));

		Render::PolygonOutline(3, MouseVt, Color(255, 255, 255, 230), Color(255, 255, 255, 240));
	}
}
...
bool CGUI::DrawWindow(CWindow* window)
{
	...
	//Render::Clear(window->m_x + 2, window->m_y + 2 + 26, window->m_iWidth - 4, window->m_iHeight - 4 - 26, Color(20, 20, 20, 255));
	//Render::Outline(window->m_x + 1, window->m_y + 1, window->m_iWidth - 2, window->m_iHeight - 2, Color(1, 1, 1, 220));
	// Main Window
	/*Render::Outline(window->m_x, window->m_y, window->m_iWidth, window->m_iHeight, Color(21, 21, 21, 80));
	...
	Render::Clear(window->m_x + 8, window->m_y + 1 + 27, window->m_iWidth - 4 - 12, window->m_iHeight - 2 - 8 - 26, Color(100, 100, 100, 200));
	...
	if (TabCount) // If there are some tabs
	{
		...
		for (int i = 0; i < TabCount; i++)
		{
			...
			Render::Clear(window->m_x + 8, window->m_y + 1 + 27, window->m_iWidth - 4 - 12, 2, Color(150, 150, 150, 255));
		}
	}
